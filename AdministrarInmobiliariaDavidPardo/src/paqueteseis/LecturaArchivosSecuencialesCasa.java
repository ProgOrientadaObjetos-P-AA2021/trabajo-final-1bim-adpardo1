...
public class LecturaArchivosSecuencialesCasa {
    ...
    public String toString() {
        ...
        for (int i = 0; i < obtenerListaCasas().size(); i++) {
            ...
            if(obtenerListaCasas().get(i).getPropietario().contains(id))
            cadena = String.format("%s %s - %s - %s - %s - %s - %s - %s - %s\n", cadena,
                    p.getPropietario(), p.getPrecioMEtroCuadrado(), p.getNumeroCuartos()
            , p.getPrecioFinal(), p.getUbicacionCasa(), p.getCiudadCasa(),p.getNumeroCuartos()
            ,p.getConstructoraCasa());
            l=1;
        }
        if(l!=1){
